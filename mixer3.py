...
class SimultaneousWAVPlayer:
    def __init__(self, file_list):
        ...
        self.sample_rates = []
        self.mute_states = []
        self.volume_levels = []  # New attribute for volume control
        ...
        for file in file_list:
            ...
            self.volume_levels.append(1.0)  # Default volume to 1.0 (full volume)
    ...
    def _audio_callback(self, output, frames, time, status):
        ...
        #slicing into chunks
        chunks = [
            data[self.pos:end] * (0 if mute else volume)
            for data, mute, volume in zip(self.data, self.mute_states, self.volume_levels)
        ]
    ...
    def set_volume(self, index, volume):
        ...
        :param index: index of the audio file to adjust volume
        :param volume: volume level between 0.0 and 1.0
        """
        if 0 <= index < len(self.volume_levels):
            if 0 <= volume <= 1:
                self.volume_levels[index] = volume
                print(f"File {index + 1} volume set to: {volume}")
            else:
...
def main():
    ...
    try:
        ...
        while True:
            ...
            cmd = input(f"commands: {cmd_options}, v[number]=[volume] (set volume), m[number] (mute), q (quit): ")
            ...
            try:
                ...
                if cmd.startswith('v'):
                    ...
                    if len(parts) == 2:
                        try:
                            ...
                            player.set_volume(index, volume)
                        except ValueError:
                            print("Invalid volume input. Use format v[number]=[volume]")
                ...
                # Mute toggling
                    ...
                    try:
                        ...
                        print("Invalid mute input. Use format m[number]")
                
                # Direct track selection (optional)
                else:
                    try:
                        index = int(cmd) - 1
                        ...
                        print("Invalid input. Please enter a number, v[number]=[volume], m[number], or 'q'.")
